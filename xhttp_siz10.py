...
async def packet_up_upload(uuid: str, session_id: str, seq: int, request: Request):
    ...
    if not uid:
        raise HTTPException(status_code=404, detail="not found")
    sess = await _get_or_create_session(uid, "packet-up", session_id, _req_client_ip(request))
    if sess.get("closed"):
        raise HTTPException(status_code=404, detail="session closed")
    ...
    if not await check_and_use(uid, len(body)):
    ...
    try:
        if sess["writer"] is None:
            ...
            await _open_tcp_for_session(session_id, uid, sess, body)
            nxt = 1
...
async def stream_up_upload(uuid: str, session_id: str, request: Request):
    ...
    if not uid:
        raise HTTPException(status_code=404, detail="not found")
    sess = await _get_or_create_session(uid, "stream-up", session_id, _req_client_ip(request))
    if sess.get("closed"):
        raise HTTPException(status_code=404, detail="session closed")
    ...
    async with upload_lock:  # جلوگیری از نوشتن هم‌زمان دو POST روی یک writer
        ...
        try:
            async for chunk in request.stream():
                ...
                if writer is None:
                    await _open_tcp_for_session(session_id, uid, sess, chunk)
                    writer = sess["writer"]
                    continue
...
# ── Alias: path ساده /{token}/... به‌جای /xhttp-siz10/{mode}/{uuid}/... ─────────
@router.get("/{token}/{session_id}")
async def xhttp_by_path_downlink(token: str, session_id: str, request: Request):
    """دانلینک با path رندوم؛ mode از protocol کانفیگ خوانده می‌شود."""
    if token in ("xhttp-siz10", "api", "ws", "ss", "trojan-ws", "sub", "login", "dashboard"):
    ...
    proto = str(link.get("protocol") or "xhttp-stream-up")
...
@router.post("/{token}/{session_id}/{seq}")
async def xhttp_by_path_packet_up(token: str, session_id: str, seq: int, request: Request):
    if token in ("xhttp-siz10", "api", "ws", "ss"):
        raise HTTPException(status_code=404)
    # reuse packet-up by resolving token→uuid
    return await packet_up_upload(token, session_id, seq, request)


@router.post("/{token}/{session_id}")
async def xhttp_by_path_stream_up(token: str, session_id: str, request: Request):
    if token in ("xhttp-siz10", "api", "ws", "ss"):
        raise HTTPException(status_code=404)
    return await stream_up_upload(token, session_id, request)
